...
#source('R/metadata.R')#loads dataset-specific variables

#make dataframes
df_input <- bespoke_data
df_numeric <- bespoke_data %>% select_if(~is.numeric(.) & length(unique(.)) > 10) %>% select(-contains('id'))
df_character <- bespoke_data %>% select_if(~is.character(.)) %>% select(-contains('id'))
df_id <- bespoke_data %>% select(contains('id')) #make sure a variable contains 'id'!!!
df_joined_character <- bespoke_data_join %>% select_if(~is.character(.)) %>% select(-contains('id'))
df_joined <- joined
#do I use these?
...
df_numeric1_vec <- bespoke_data %>% select(matches(df_numeric1_name)) %>% pull()
df_numeric2_name <- df_numerics[2]
df_numeric2_vec <- bespoke_data %>% select(matches(df_numeric2_name)) %>% pull()
df_characters <- sample(colnames(df_character), 2)
df_char1_name <- df_characters[1]
df_char1_vec <- bespoke_data %>% select(matches(df_char1_name)) %>% pull() #not used
df_char2_name <- df_characters[2]
df_char2_vec <- bespoke_data %>% select(matches(df_char2_name)) %>% pull() #not used
df_id_name <- sample(colnames(df_id), 1)
df_id_vec <- bespoke_data %>% select(matches(df_id_name)) #not used
