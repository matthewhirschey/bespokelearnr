...
- pattern we want to count `r word1`  

`str_count(```r dataframe_join_name`$`r df_2_col```, ```r word1```)`

## Counting
`str_count(```r dataframe_join_name`$`r df_2_col```, ```r word1```)`

```{r str_count example, echo= FALSE, warning = FALSE}
str_count(df_input2[df_2_col], word1)
```
...
`str_count(```r dataframe_join_name`$`r df_2_col```, ```r word2```)` 
...
`str_detect(```r dataframe_join_name`$`r df_2_col```, "Diabetic")`  
...
In the df_2_col column we have:  
-`"Diabetic"`  
-`"High Cholesterol"`  
...
`str_replace(```r dataframe_join_name`$`r df_2_col```, "Normal blood sugar and cholesterol", "Normal")`
...
## Modifiying strings with `str_replace()` {.build}
We use this same code to  modify the `df_2_col` column by assigning it to the same variable     
  
`dataframe_join_name$df_2_col <-`  
       `str_replace(dataframe_join_name$df_2_col, "Normal blood sugar and cholesterol", "normal")`
...
`dataframe_join_name2 <- dataframe_join_name %>% mutate(healthy = str_detect(df_2_col, "normal"))` 
